...
export default function AuthLoginForm(): ReactNode {
  ...
  const handleClick = () => {
    toast({
      title: "Coming soon !",
      description:
        "You will be able to login with github soon. For now you can create a issue on Github if you want to make modifications on the website.",
      variant: "destructive",
    });
  };
  ...
  return (
    <>
          ...
          <Form {...loginForm}>
            <form onSubmit={loginForm.handleSubmit(handleSubmit)} className={"space-y-4"}>
              ...
              <Button
                ...
                variant="default"
                className="w-full"
                disabled={loginForm.formState.isSubmitting || pending}
              >
                Login
              </Button>
            </form>
      ...
      <Link href={"/register"} className={cn(buttonVariants({ variant: "link" }), "absolute right-8 top-8")}>
        Register
      </Link>
    </>
  );
