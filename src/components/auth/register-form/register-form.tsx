...
export default function RegisterForm(): ReactNode {
  ...
  const handleClick = () => {
    toast({
      title: "Coming soon !",
      description:
        "You will be able to register soon. For now you can create a issue on Github if you want to make modifications on the website.",
      variant: "destructive",
    });
  };
    ...
    <>
      <div className="mx-auto flex h-full w-full flex-col justify-center space-y-6 sm:w-[350px]">
        ...
        <div className={"grid gap-6"}>
          ...
          <Button variant="outline" className="w-full cursor-not-allowed" onClick={handleClick}>
        ...
        <p className="px-8 text-center text-sm text-muted-foreground">
          ...
          <Link className="underline underline-offset-4 hover:text-primary" href={`${link}/terms-of-service`}>
            Terms of Service
          </Link>{" "}
          and{" "}
          <Link className="underline underline-offset-4 hover:text-primary" href={`${link}/privacy-policy`}>
            Privacy Policy
          </Link>
          .
        </p>
      </div>
      <Link href={"/login"} className={cn(buttonVariants({ variant: "link" }), "absolute right-8 top-8")}>
        Login
      </Link>
    </>
  );
